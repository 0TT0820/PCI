    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.epochs_since_last_save >= self.period:
            ...
            if self.save_best_only:
                ...
                if current is None:
                    self.out_info('Can save best model only with %s available, skipping.' % self.monitor)
                else:
                    if self.monitor_op(current, self.best):
                        if self.verbose > 0:
                            self.out_info('Epoch %05d: %s improved from %0.5f to %0.5f,'
                        ...
                        if self.verbose > 0:
                            self.out_info('Epoch %05d: %s did not improve from %0.5f' %
